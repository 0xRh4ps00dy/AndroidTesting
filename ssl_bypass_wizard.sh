#!/usr/bin/env bash
# =============================================================================
# ssl_bypass_wizard.sh — Wizard interactivo para bypass de SSL Pinning en AVDs
# Basado en: https://www.mfumis.com/posts/bypassing-ssl-pinning-on-play-store-avds-without-frida/
# Uso: chmod +x ssl_bypass_wizard.sh && ./ssl_bypass_wizard.sh
# =============================================================================
...
step_5_install_cert() {
    ...
    local inject_script
    inject_script="$(dirname "$(realpath "$0")")/inject_cert_android.sh"

    if [ ! -f "$inject_script" ]; then
        warn "No se encontró inject_cert_android.sh junto al wizard."
        warn "Generando el script de inyección en /tmp/inject_cert_android.sh..."
        _write_inject_script "/tmp/inject_cert_android.sh"
        inject_script="/tmp/inject_cert_android.sh"
    fi

    run_cmd adb push "$inject_script" /storage/self/primary/inject_cert_android.sh
    ...
    run_cmd adb shell su -c "sh /storage/self/primary/inject_cert_android.sh $CERT_ANDROID_NAME"
...
#!/system/bin/sh
CERT_FILENAME="$1"
if [ -z "$CERT_FILENAME" ]; then echo "[!] Uso: inject_cert_android.sh <hash>.0"; exit 1; fi
...
step_final_summary() {
    ...
    cmd_hint "adb shell su -c \"sh /storage/self/primary/inject_cert_android.sh $CERT_ANDROID_NAME\""
