...
Antes de ejecutar los scripts, asegúrate de cumplir con los siguientes requisitos en tu máquina host (Linux, macOS o Windows):
...
4. Las dependencias `adb`, `openssl` y `xz-utils` instaladas en tu sistema host.
   * **En Windows:** 
     * El script de PowerShell busca automáticamente `openssl` en el PATH y, si no lo encuentra, intentará autodetectarlo en la ruta de instalación por defecto de **Git para Windows** (`C:\Program Files\Git\usr\bin\openssl.exe`).
     * Si no tienes Git, puedes instalar OpenSSL ejecutando `winget install --id=ShiningLight.OpenSSL` o `winget install --id=Git.Git` y abriendo una terminal nueva.
     * La dependencia `xz-utils` no es necesaria en Windows, ya que el script de Frida utiliza Python de forma nativa para descomprimir `.xz`.
...
Para configurar el bypass de SSL Pinning tradicional e inyectar el certificado CA en el almacén de credenciales del emulador:
...
> [!IMPORTANT]
> **Políticas de Ejecución en Windows (PowerShell):**
> Si recibes un error indicando que la ejecución de scripts está deshabilitada en el sistema, puedes habilitarla temporalmente para tu consola actual ejecutando:
> ```powershell
> Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
> ```
> O bien, ejecutar el script pasándole el parámetro de bypass directamente:
> ```powershell
> powershell -ExecutionPolicy Bypass -File .\bypass_ssl_directo.ps1 -Cert C:\Users\usuario\Documents\burpsuite.der -Type burp -ProxyIp 192.168.1.137 -ProxyPort 8080
> ```
...
# Ejemplo para YouTube:
.\iniciar_frida_bypass.ps1 com.google.android.youtube
```
...
* **`asistente_bypass_ssl.sh`:** Asistente interactivo guiado por menús en la terminal (solo Linux/macOS).
* **`bypass_ssl_directo.sh` / `bypass_ssl_directo.ps1`:** Script principal por argumentos en línea de comandos.
* **`conmutar_proxy.sh` / `conmutar_proxy.ps1`:** Activa y desactiva con un solo comando la redirección de tráfico al proxy interceptor.
* **`instalar_aurora.sh` / `instalar_aurora.ps1`:** Instala de forma rápida **Aurora Store** (tienda alternativa para descargar apps de Google Play).
* **`instalar_xapk.sh` / `instalar_xapk.ps1`:** Automatiza la descompresión e instalación de paquetes múltiples (`.xapk`, `.apks`, `.zip`) forzando la procedencia desde Google Play (`com.android.vending`) y filtrando los splits de arquitectura de CPU no compatibles con el emulador.
* **`iniciar_frida_bypass.sh` / `iniciar_frida_bypass.ps1`:** Automatiza el arranque de Frida y la inyección en caliente de `bypass-ssl.js`.
...
### Windows (PowerShell):
```powershell
.\bypass_ssl_directo.ps1 disconnect
# O bien:
.\conmutar_proxy.ps1
```
